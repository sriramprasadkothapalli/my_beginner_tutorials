// Copyright (c) 2024 Sriramprasad Kothapalli
...
/**
 * @class TalkerNode
 * @brief A ROS2 node that publishes messages to the "chatter" topic and provides a service to change the base message string.
 */
class TalkerNode : public rclcpp::Node {
 public:
  ...
  : Node("talker"), base_output_string_("Hello ROS2") {
    ...
    service_ = this->create_service<example_interfaces::srv::SetBool>(
        ...
        std::bind(&TalkerNode::handle_change_output_string, this, std::placeholders::_1, std::placeholders::_2));

    RCLCPP_INFO(this->get_logger(), "TalkerNode has been started.");
    RCLCPP_DEBUG(this->get_logger(), "Debugging information: Node initialized.");
    RCLCPP_WARN(this->get_logger(), "Warning: This is a simple warning at startup.");
  }
 ...
 private:
  ...
  void publish_message() {
    ...
    if (count_ > 20) {
      RCLCPP_ERROR(this->get_logger(), "Count has exceeded 20, something might be wrong!");
    }

    if (count_ > 50) {
      RCLCPP_FATAL(this->get_logger(), "Count has exceeded 50, shutting down node.");
      rclcpp::shutdown();
    }
  }
   ...
   * @param request The request containing a boolean indicating whether to change the base string.
   * @param response The response indicating success or failure of the operation.
   */
  void handle_change_output_string(const std::shared_ptr<example_interfaces::srv::SetBool::Request> request,
  ...
  rclcpp::Publisher<std_msgs::msg::String>::SharedPtr publisher_; ///< Publisher for the "chatter" topic.
  rclcpp::TimerBase::SharedPtr timer_; ///< Timer to periodically publish messages.
  rclcpp::Service<example_interfaces::srv::SetBool>::SharedPtr service_; ///< Service to change the base output string.
  size_t count_; ///< Counter for the number of messages published.
  std::string base_output_string_; ///< The base string used in published messages.
};
